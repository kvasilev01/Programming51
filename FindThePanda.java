...
public class FindThePanda {
	...
	private boolean isAllowed(int[] position) {
		if ((position[0] > maze.length - 1) || (position[0] < 0))
			return false;
		if ((position[1] > maze[0].length - 1) || (position[1] < 0))
			return false;
		if (maze[position[0]][position[1]] == wallSymbol)
			return false;
		if (maze[position[0]][position[1]] == pastSymbol)
			return false;
		return true;
	}
	...
	public void traverseRecursivly(int[] position) {
		...
		if (isAllowed(Up(position)))
			traverseRecursivly(Up(position));
		if (isAllowed(Left(position)))
			traverseRecursivly(Left(position));
		if (isAllowed(Down(position)))
			traverseRecursivly(Down(position));
		if (isAllowed(Right(position)))
			traverseRecursivly(Right(position));
	}
